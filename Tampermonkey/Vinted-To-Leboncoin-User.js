// ==UserScript==
// @name         Vinted → Leboncoin - L'extension du siècle ⚡️
...
(function(){
  console.log("[V>L Tampermonkey] loaded", location.href);
  ...
  function injectVintedButton(){
    ...
    if(!h1) return;
    const data = getListingFromVinted();
    ...
    btn.onclick=()=>{
      ...
      // copy description to clipboard too
    ...
    console.log("[V>L] bouton injecte");
  }
  ...
  async function injectLeboncoinPanel(){
    ...
    panel.innerHTML=`
      <div style="position:fixed;top:80px;right:20px;z-index:999999;background:white;border:2px solid #ff6e14;border-radius:16px;padding:16px;width:360px;box-shadow:0 8px 30px rgba(0,0,0,.2);font-family:Inter,sans-serif">
        <div style="font-weight:900;font-size:16px;margin-bottom:8px">⚡️ Tampermonkey - Vinted detectee</div>
        <div style="font-size:13px;color:#555;margin-bottom:12px"><b>${data.title}</b><br>${data.price} EUR • ${data.images?.length||0} photos</div>
        <div id="photo-status-tm" style="font-size:12px;background:#fff3e0;padding:8px;border-radius:8px;margin-bottom:10px">Pret a remplir</div>
        ...
        <button id="tm-photos" style="width:100%;background:#111;color:white;border:none;padding:12px;border-radius:10px;font-weight:800;cursor:pointer">2. Telecharger photos</button>
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:10px">${(data.images||[]).map((s,i)=>`<img src="${s}" style="width:56px;height:56px;object-fit:cover;border-radius:8px;border:2px solid #ff6e14">`).join('')}</div>
        <div style="font-size:10px;color:#666;margin-top:8px">💡 Tampermonkey peut telecharger auto les photos dans ton dossier</div>
      </div>`;
    ...
    document.getElementById('tm-fill').onclick=()=>{
      ...
      if(subject){ subject.focus(); subject.value=data.title; subject.dispatchEvent(new Event('input',{bubbles:true})); subject.dispatchEvent(new Event('change',{bubbles:true})); }
      ...
      document.getElementById('photo-status-tm').innerText='✅ Texte rempli ! Maintenant telecharge les photos.';
    };

    document.getElementById('tm-photos').onclick=()=>{
      (data.images||[]).forEach((url,i)=>{
        GM_download({url, name: `vinted-import/vinted-${i+1}.jpg`, onload:()=>console.log('dl ok',i)});
      });
      document.getElementById('photo-status-tm').innerText='📥 Photos en telechargement dans Telechargements/vinted-import/ - Glisse-les sur Leboncoin';
    };
  }
