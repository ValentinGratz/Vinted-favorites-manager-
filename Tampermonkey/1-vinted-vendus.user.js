...
// @version      1.6.2
// @description  [FAVORIS] Détecte les articles "Vendu" dans tes favoris et les supprime en masse. Mode ULTRA pour 2000+ favs.
// @author       ValentinGratz
// @match        https://www.vinted.fr/member/items/favourite_list*
// @match        https://www.vinted.com/member/items/favourite_list*
// @match        https://www.vinted.de/member/items/favourite_list*
// @match        https://www.vinted.es/member/items/favourite_list*
// @match        https://www.vinted.it/member/items/favourite_list*
// @match        https://www.vinted.nl/member/items/favourite_list*
// @match        https://www.vinted.pl/member/items/favourite_list*
// @grant        none
...
(function() {
    ...
    async function start(){
        ...
        const sold=all.filter(el=>el.querySelector('[data-testid*="status-text"]')?.textContent.trim()==='Vendu');
        if(!sold.length){ alert('✅ Aucun vendu trouvé'); return; }
        sold.forEach(el=>{ el.style.outline='4px solid #ff4444'; el.style.position='relative'; const badge=document.createElement('div'); badge.textContent='🔴 VENDU'; badge.style.cssText='position:absolute;top:8px;left:8px;background:#ff4444;color:white;padding:5px 10px;border-radius:20px;font-size:11px;font-weight:bold;z-index:50'; el.appendChild(badge); });
        const panel=document.createElement('div'); panel.style.cssText='position:fixed;bottom:20px;right:20px;background:#1e1e1e;color:white;padding:18px;border-radius:12px;z-index:99999;width:360px;border:2px solid #ff4444;font-family:sans-serif;';
