...
// @version      1.6.2
// @description  [DRESSING] Met en favori automatiquement les articles d'un dressing (mode humain avec délais aléatoires anti-détection). S'arrête tout seul après 40 favs.
...
(function() {
    ...
    async function start(){
        ...
        const randomDelay=()=> 1500 + Math.random()*2500; // 1.5s à 4s entre chaque fav = mode humain

        // 1. Pre-scroll complet
        const bar=document.createElement('div'); bar.style.cssText='position:fixed;top:0;left:0;width:100%;background:#FFB800;color:black;padding:8px;text-align:center;z-index:9999999;font-family:sans-serif;font-size:13px;font-weight:bold'; bar.textContent='⏳ Chargement complet du dressing avant auto-fav...'; document.body.appendChild(bar);
        ...
        const panel=document.createElement('div'); panel.style.cssText='position:fixed;bottom:20px;right:20px;background:#1e1e1e;color:white;padding:18px;border-radius:12px;z-index:99999;width:380px;border:2px solid #FFB800;font-family:sans-serif;';
        ...
        let count=0;
        const limit=Math.min(toFav.length, 40); // LIMITE DE SECURITE
        ...
        for(let i=0;i<limit;i++){
            ...
            // petit effet visuel
            const badge=document.createElement('div'); badge.textContent='⭐ FAV'; badge.style.cssText='position:absolute;top:8px;left:8px;background:#FFB800;color:black;padding:5px 10px;border-radius:20px;font-size:11px;font-weight:bold;z-index:50'; o.item.style.position='relative'; o.item.appendChild(badge);
